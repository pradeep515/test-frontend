...
class QueryState(BaseState):
    ...
    def make_request(self):
        logger.debug(f"Making {self.current_req} request to {self.req_url}")
        # Add more logging as needed in your request handling logic
        # For example:
        # logger.debug(f"Request headers: {self.headers}")
        # logger.debug(f"Request body: {self.body}")
        # logger.debug(f"Request cookies: {self.cookies}")
        # response = httpx.request(self.current_req, self.req_url, headers=self.headers, json=self.body, cookies=self.cookies)
...
class QueryAPI(QueryState):
    ...
    async def run_get_request(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            self.get_data = res.json()
            logger.debug(f"Response: {self.get_data}")  
            self.number_of_rows = len(self.get_data)
            logger.debug(f"Response Length: {self.number_of_rows}")  
